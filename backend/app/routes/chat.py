...
def list_conversations(db: Session = Depends(get_db)):
    """Lists all past chat conversations."""
...
def create_conversation(payload: ConversationCreate, db: Session = Depends(get_db)):
    """Creates a new conversation session, optionally attaching a PDF."""
...
def update_conversation(
    ...
    """Updates conversation properties (e.g. title or attaching/detaching a document)."""
...
def clear_conversation_messages(conversation_id: int, db: Session = Depends(get_db)):
    """Clears all messages in a specific conversation session."""
    conv = db.query(Conversation).filter(Conversation.id == conversation_id).first()
    if not conv:
        raise HTTPException(status_code=404, detail="Conversation not found")
...
def delete_last_message(conversation_id: int, db: Session = Depends(get_db)):
...
def chat_endpoint(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    search_results = None
    system_instruction = (
        "You are BharatAI, a premium AI Assistant designed for users in India and globally. "
    ...
    # 6. Generate Response with local offline fallbacks for 429 errors
    try:
        ...
        err_msg = str(e)
        print(f"Chat generation failed, attempting local fallback: {err_msg}")
        is_rate_limited = "429" in err_msg or "quota" in err_msg.lower() or "limit" in err_msg.lower() or "exhausted" in err_msg.lower()
        
        if is_rate_limited and conv.document_id:
            doc = db.query(DocumentMetadata).filter(DocumentMetadata.id == conv.document_id).first()
            if doc:
                try:
                    ...
                    # Split full text into sentences or paragraphs
                    ...
                    if excerpts:
                        excerpt_text = "\n\n• ".join(excerpts)
                        ai_response = (
                            "⚠️ **[Local Offline Search Mode: Gemini API limit reached (429)]**\n\n"
                            f"I've searched the document **'{doc.filename}'** locally for your query. "
                            "Here are the most relevant matching excerpts:\n\n"
                            f"• {excerpt_text}\n\n"
                            "*(Live AI answers will resume once the Google API limit reset window passes)*"
                        )
                    else:
                        # Fallback to pre-computed summary
                        ai_response = (
                            "⚠️ **[Local Offline Mode: Gemini API limit reached (429)]**\n\n"
                ...
                ai_response = "⚠️ **[Gemini API Rate-Limit (429)]**: You have exceeded the free tier daily request limit. Please try again in a minute."
        else:
            ai_response = f"⚠️ **[Gemini API Rate-Limit (429)]**: You have exceeded the free tier daily request limit. Please try again shortly."
